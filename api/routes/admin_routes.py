...
def get_login_logs():
    try:
        ...
        if status_filter in ('ALLOW', 'FLAG', 'BLOCK'):
            cur.execute("""
                SELECT id, user_id, email, ip_address, device_info, city, country,
                       ...
                       total_risk, decision, face_verdict, face_confidence, timestamp
        ...
        logs = [{
            ...
            'face_confidence': r['face_confidence'], 'timestamp': r['timestamp']
        } for r in rows]
...
def get_stats():
    try:
        ...
        cur.execute("SELECT AVG(device_risk) as a, AVG(location_risk) as b, AVG(behavior_risk) as c, AVG(face_risk) as d FROM login_logs")
        ...
        cur.execute("""
            SELECT id, email, total_risk, decision, city, country, face_verdict, face_confidence, timestamp
        ...
        return jsonify({
            'total_logins': total, 'allowed': decision_counts.get('ALLOW', 0),
            'flagged': decision_counts.get('FLAG', 0), 'blocked': decision_counts.get('BLOCK', 0),
            'avg_risk': avg_risk,
            ...
            'deepfakes_blocked': deepfakes_blocked, 'real_faces': real_faces,
